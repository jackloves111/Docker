<template>
  <div>
    ...
    <a-modal
      ...
      width="700px"
    >
      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px; padding: 12px; background: #f6ffed; border-radius: 4px;" v-if="resultData.success">
        <span style="font-size: 24px;">✅</span>
        <span style="font-size: 16px; font-weight: bold; color: #52c41a;">执行成功</span>
      </div>
      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px; padding: 12px; background: #fff2f0; border-radius: 4px;" v-else>
        <span style="font-size: 24px;">❌</span>
        <span style="font-size: 16px; font-weight: bold; color: #ff4d4f;">执行失败</span>
      </div>
...
const getStepDescription = (step) => {
  ...
  if (step.type === 'image_load') return config.url ? (config.url.length > 30 ? config.url.substring(0, 27) + '...' : config.url) : ''
...
// Extract variables from batch items
