      ...
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item v-if="form.type === 'run'" label="启动命令" required>
          <a-textarea
            ...
            :rows="6"
            placeholder="docker run -d --name myapp -p 80:80 -v /data:/app/data nginx:latest"
...
services:
  web:
    ...
    volumes:
      ...
      - ${CONFIG_ROOT}/nginx/nginx.conf:/etc/nginx/nginx.conf"
            style="font-family: monospace"
          />
        </a-form-item>
        ...
        <a-form-item>
          ...
          <a-button style="margin-left: 8px" @click="$router.back()">
            取消
          </a-button>
        </a-form-item>
      </a-form>
...
const handleSave = async () => {
  ...
  saving.value = true
  try {
    await projectsAPI.update(projectId.value, form.value)
